...
// untrusted hosts. Used by pin's source constructors when no caller-
// supplied client is available.
//
// Three defences, all in concert:
//
...
//     CGNAT 100.64/10, link-local, multicast, unspecified) before any
//     TCP connect. The connection then dials the resolved IP directly,
//     so a rebind between check and connect cannot escape the gate.
//
//  2. Redirect cap. Go's default ignores chain length and re-trusts
//     each hop. We cap at 10 and re-validate every redirect target.
//
//  3. Scheme gate on redirect. file://, gopher://, ftp:// and friends
//     are rejected — the only reason a registry would 30x to those is
//     to exfiltrate something.
//
// Tests that want to hit an httptest.Server have to either pass their
// own *client.Client (bypassing this wrapper entirely) or supply an
// Options.AllowLoopback escape hatch.
package safehttp
...
// testInsecure, when true, flips both AllowLoopback and AllowPrivate on
// at the gate. Set via EnableLoopbackForTesting from a test binary's
// TestMain so existing httptest-based tests don't have to thread an
